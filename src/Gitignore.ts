/**
 * Port of the 'ignore' npm package (https://github.com/kaelzhang/node-ignore)
 * This is a 1:1 port to avoid external dependencies
...
const TRAILING_WILD_CARD_REPLACERS: Record<
  Mode,
...
class IgnoreRule {
  ...
  _make(mode: Mode, key: string): RegExp {
    ...
    const regex = (this as any).ignoreCase
      ? new RegExp(str, "i")
      : new RegExp(str)
...
const throwError = (
  message: string,
  Ctor: ErrorConstructor | RangeErrorConstructor | TypeErrorConstructor,
): never => {
...
export class Ignore {
  ...
  constructor(
    { ignorecase = true, ignoreCase = ignorecase, allowRelativePaths = false }:
      IgnoreOptions = {},
  ) {
