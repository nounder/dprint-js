...
import { loadPlugins } from "../formatter.js";
import * as path from "node:path";

/**
 * Output the resolved configuration for the plugins
 * @param {object} options - Command options
 * @returns {Promise<number>} Exit code (0 for success, 11 for config error, 13 for plugin error)
 */
export default async function outputResolvedConfigCommand(options = {}) {
  ...
  for (const plugin of loadedPlugins) {
    try {
      ...
      if (shouldLog("warn")) {
        console.warn(`Warning: Could not get resolved config for ${plugin.name}: ${error.message}`);
      }
    }
  }
