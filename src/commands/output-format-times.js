...
import { loadPlugins, getFormatterForFile, formatText } from "../formatter.js";
 ...
 */
export default async function outputFormatTimesCommand(patterns = [], options = {}) {
  ...
  let files;
  try {
    files = await findFiles(config, patterns, cwd, options);
  ...
  for (const file of files) {
    ...
    try {
      const formatter = getFormatterForFile(absolutePath, loadedPlugins);
      if (!formatter) {
      ...
      formatText(absolutePath, content, formatter);
      ...
      if (shouldLog("error")) {
        console.error(`Error formatting ${absolutePath}: ${error.message}`);
      }
    }
  }
