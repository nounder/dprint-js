...
export async function fmtCommand(
  ...
  const { plugins: loadedPlugins, autoDiscovered }:
    Formatter.LoadPluginsResult = await Formatter.loadPlugins(
      config,
      cwd,
    )
  ...
  // Find files
  const files = await Files.findFiles(config, filePatterns, cwd)
...
export async function checkCommand(
  ...
  for (const file of files) {
    const result = await Formatter.formatFile(file, loadedPlugins, config, true)
