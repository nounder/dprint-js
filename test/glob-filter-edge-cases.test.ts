...
import { findMatchingFiles } from "../src/glob.js";
import { normalizeExcludePatterns } from "../src/glob.js";
import * as fs from "node:fs";
import * as path from "node:path";
import * as os from "node:os";

t.describe("Bun glob filter edge cases", () => {
  t.it("should not filter files with consecutive dots in filename", async () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-dots-test-"));
    ...
    try {
      ...
      const files = findMatchingFiles(["**/*"], [], testDir);
  ...
  t.it("should handle dot files correctly", async () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-dotfiles-test-"));
    ...
    try {
      ...
      const files = findMatchingFiles([".gitignore", ".config.js", ".hidden/**/*.ts"], [], testDir);
  ...
  t.it("should not break with nested directories", async () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-nested-test-"));

    try {
      // Create deeply nested structure
      fs.mkdirSync(path.join(testDir, "a", "b", "c", "d"), { recursive: true });
      fs.writeFileSync(path.join(testDir, "a", "b", "c", "d", "deep.ts"), "// deep file");
      fs.writeFileSync(path.join(testDir, "a", "mid.ts"), "// mid file");

      const files = findMatchingFiles(["**/*.ts"], [], testDir);
  ...
  t.it("should handle patterns with dots correctly", async () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-pattern-dots-test-"));

    try {
      fs.mkdirSync(path.join(testDir, ".config"));
      fs.writeFileSync(path.join(testDir, ".config", "app.json"), "{}");
      fs.writeFileSync(path.join(testDir, ".config", "settings.ts"), "export {}");
      ...
      const files = findMatchingFiles([".config/**/*", ".vscode/**/*"], [], testDir);
  ...
  t.it("should filter paths with /../ in the middle", async () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-middle-escape-test-"));
  ...
  t.it("should respect exclude patterns after filtering", async () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-exclude-test-"));
    ...
    try {
      ...
      const excludes = normalizeExcludePatterns(["test/**"]);
      const files = findMatchingFiles(["**/*.ts"], excludes, testDir);
  ...
  t.it("should handle patterns that would trigger Bun bug without escaping", async () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-bun-bug-test-"));
    ...
    try {
      ...
      for (const pattern of vulnerablePatterns) {
        const files = findMatchingFiles([pattern], [], testDir);
  ...
  t.it("should handle mixed safe and unsafe patterns", async () => {
    ...
    try {
      ...
      fs.mkdirSync(path.join(testDir, ".config"));
      fs.writeFileSync(path.join(testDir, ".config", "settings.ts"), "// config");
  ...
  t.it("should handle special characters in filenames", async () => {
    const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-special-test-"));
