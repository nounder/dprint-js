...
t.it("security: vulnerable glob patterns are silently filtered", async () => {
  const testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-security-test-"));
  ...
  try {
    ...
    const files1 = await Files.findFiles(config1, [], path.join(testDir, "project"));
    ...
    const files2 = await Files.findFiles(config2, [], path.join(testDir, "project"));
    ...
    for (const pattern of vulnerablePatterns) {
      ...
      const files = await Files.findFiles(config, [], path.join(testDir, "project"));
