...
t.describe("circular symlink handling", () => {
  t.it("handles circular symlinks gracefully without crashing", async () => {
    ...
    try {
      fs.mkdirSync(path.join(testDir, "examples", "node_modules"), {
        recursive: true,
      })
      fs.symlinkSync(
        testDir,
        path.join(testDir, "examples", "node_modules", "project"),
        "dir",
      )
    } catch (error) {
    ...
    const demo = fs.readFileSync(
      path.join(testDir, "examples", "demo.ts"),
      "utf-8",
    )
  ...
  t.it("check command handles circular symlinks gracefully", async () => {
    ...
    try {
      fs.mkdirSync(path.join(testDir, "lib", "node_modules"), {
        recursive: true,
      })
      fs.symlinkSync(
        testDir,
        path.join(testDir, "lib", "node_modules", "self"),
        "dir",
      )
    } catch (error) {
  ...
  t.it("node_modules exclusion prevents circular symlink issues", async () => {
    ...
    try {
      ...
      fs.symlinkSync(
        testDir,
        path.join(testDir, "node_modules", "pkg", "circular"),
        "dir",
      )
    } catch (error) {
      if (error.code === "EPERM" || error.code === "EACCES") {
  ...
  t.it("deep nested symlink structures are handled", async () => {
    ...
    fs.mkdirSync(path.join(testDir, "packages", "app", "src"), {
      recursive: true,
    })
    fs.writeFileSync(
      path.join(testDir, "packages", "app", "src", "main.ts"),
      "const x = 1;",
    )

    // Create nested symlink that could cause very long paths
    try {
      fs.mkdirSync(path.join(testDir, "packages", "app", "node_modules"), {
        recursive: true,
      })
      fs.symlinkSync(
