...
import * as OutputFormatTimesCommand from "../../src/commands/OutputFormatTimesCommand.ts"
...
t.beforeEach(() => {
  testDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "dprint-test-comparison-output-format-times-"),
  )
...
t.it("outputs timing for formatted files", async () => {
  ...
  const ourResult = await $`${OURS_BIN} output-format-times`
    .cwd(oursDir)
    .nothrow()
    .quiet()
  const theirResult = await $`${THEIR_BIN} output-format-times`
    .cwd(theirsDir)
    .nothrow()
    .quiet()
...
t.it("returns same exit code when no files found", async () => {
  ...
  const theirResult =
    await $`${THEIR_BIN} output-format-times --log-level silent`
      .cwd(theirsDir)
      .nothrow()
      .quiet()
...
t.it("returns same exit code when config is missing", async () => {
  ...
  const theirResult =
    await $`${THEIR_BIN} output-format-times --log-level silent --config dprint.json`
      .cwd(
        theirsDir,
      )
      .nothrow()
