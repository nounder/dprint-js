...
import outputFormatTimesCommand from "../../src/commands/output-format-times.js";
...
t.it("returns same exit code when no files found", async () => {
  const ourExitCode = await outputFormatTimesCommand([], { logLevel: "silent", cwd: oursDir });
...
t.it("returns same exit code when config is missing", async () => {
  ...
  const ourExitCode = await outputFormatTimesCommand([], {
    logLevel: "silent",
