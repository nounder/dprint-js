...
import outputFilePathsCommand from "../../src/commands/output-file-paths.js";
...
t.it("returns exit code 0 when no files found", async () => {
  ...
  // Run our implementation
  const ourExitCode = await outputFilePathsCommand([], { logLevel: "silent", cwd: oursDir });
...
t.it("returns same exit code when config is missing", async () => {
  ...
  // Run our implementation
  const ourExitCode = await outputFilePathsCommand([], {
    logLevel: "silent",
