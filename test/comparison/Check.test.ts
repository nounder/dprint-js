...
t.beforeEach(() => {
  ...
  testDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "dprint-test-comparison-check-"),
  )
...
t.it("returns same exit code for formatted files", async () => {
  ...
  const ourExitCode = await CheckCommand.run({
    logLevel: "silent",
    cwd: oursDir,
  })

  // Check with rust dprint
  const theirResult = await $`${THEIR_BIN} check --log-level silent`
    .cwd(theirsDir)
    .nothrow()
    .quiet()
...
t.it("returns same exit code with --allow-no-files", async () => {
  ...
  // Check with rust dprint
  const theirResult =
    await $`${THEIR_BIN} check --log-level silent --allow-no-files`
      .cwd(theirsDir)
      .nothrow()
      .quiet()
  const theirExitCode = theirResult.exitCode
...
t.it("respects file patterns identically", async () => {
  ...
  const theirResult = await $`${THEIR_BIN} check --log-level silent test.ts`
    .cwd(theirsDir)
    .nothrow()
    .quiet()
  const theirExitCode = theirResult.exitCode
...
t.it("list-different outputs same file paths", async () => {
  ...
  const ourResult =
    await $`${OURS_BIN} check --list-different --log-level silent 2>&1`
      .cwd(oursDir)
      .nothrow()
      .quiet()

  // Check with rust dprint using --list-different (outputs to stderr, so capture with 2>&1)
  const theirResult = await $`${THEIR_BIN} check --list-different 2>&1`
    .cwd(theirsDir)
    .nothrow()
    .quiet()
  ...
  const ourFiles = ourOutput
    ...
    .split("\n")
    .filter(line =>
      line && (line.includes("file1.") || line.includes("file2."))
    )
...
t.it("returns same exit code when config file is missing", async () => {
  ...
  const theirResult =
    await $`${THEIR_BIN} check --log-level silent --config dprint.json`
      .cwd(theirsDir)
      .nothrow()
      .quiet()
  const theirExitCode = theirResult.exitCode
...
t.it("returns same exit code for non-existent file argument", async () => {
  ...
  const theirResult =
    await $`${THEIR_BIN} check --log-level silent non-existent-file.ts`
      .cwd(theirsDir)
      .nothrow()
      .quiet()
  const theirExitCode = theirResult.exitCode
