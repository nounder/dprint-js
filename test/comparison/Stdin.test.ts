...
t.beforeEach(() => {
  ...
  testDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "dprint-test-comparison-stdin-"),
  )
...
t.it("formats stdin with extension identically to rust dprint", async () => {
  ...
  const ourResult =
    await $`${OURS_BIN} fmt --stdin ts --log-level silent < ${inputFile}`
      .cwd(oursDir)
      .text()

  // Format with rust dprint using stdin
  const theirResult =
    await $`${THEIR_BIN} fmt --stdin ts --log-level silent < ${inputFile}`
      .cwd(theirsDir)
      .text()
...
t.it("formats stdin with filename identically to rust dprint", async () => {
  ...
  // Format with our implementation using stdin
  const ourResult =
    await $`${OURS_BIN} fmt --stdin test.ts --log-level silent < ${inputFile}`
      .cwd(oursDir)
      .text()

  // Format with rust dprint using stdin
  const theirResult =
    await $`${THEIR_BIN} fmt --stdin test.ts --log-level silent < ${inputFile}`
      .cwd(theirsDir)
      .text()
...
t.it("handles stdin with absolute file path", async () => {
  ...
  const ourResult =
    await $`${OURS_BIN} fmt --stdin ${testFilePath} --log-level silent < ${inputFile}`
      .cwd(
        oursDir,
      )
      .text()
...
t.it(
  "returns error code when no formatter found for stdin extension",
  async () => {
    ...
    try {
      await $`${OURS_BIN} fmt --stdin xyz --log-level silent < ${inputFile}`
        .cwd(oursDir)
        .quiet()
...
t.it(
  "formats stdin with info log level includes diagnostic messages",
  async () => {
    // Write input to a temporary file
    const inputFile = path.join(testDir, "input.txt")
    fs.writeFileSync(inputFile, malformattedTS)
    ...
    const stdout =
      await $`${OURS_BIN} fmt --stdin ts --log-level info < ${inputFile}`
        .cwd(oursDir)
        .text()
