...
import outputResolvedConfigCommand from "../../src/commands/output-resolved-config.js";
...
t.beforeEach(() => {
  testDir = fs.mkdtempSync(path.join(os.tmpdir(), "dprint-test-comparison-output-resolved-config-"));
...
t.it("returns same exit code when config is missing", async () => {
  ...
  const ourExitCode = await outputResolvedConfigCommand({
    logLevel: "silent",
