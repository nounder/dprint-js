...
t.beforeEach(() => {
  ...
  testDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "dprint-test-comparison-output-file-paths-"),
  )
...
t.it("lists same file paths when files exist", async () => {
  ...
  const ourResult = await $`${OURS_BIN} output-file-paths`
    .cwd(oursDir)
    .nothrow()
    .quiet()

  // Run rust dprint
  const theirResult = await $`${THEIR_BIN} output-file-paths`
    .cwd(theirsDir)
    .nothrow()
    .quiet()
...
t.it("returns exit code 0 when no files found", async () => {
  ...
  // Run rust dprint
  const theirResult = await $`${THEIR_BIN} output-file-paths --log-level silent`
    .cwd(theirsDir)
    .nothrow()
    .quiet()
...
t.it("returns same exit code when config is missing", async () => {
  ...
  // Run rust dprint
  const theirResult =
    await $`${THEIR_BIN} output-file-paths --log-level silent --config dprint.json`
      .cwd(theirsDir)
      .nothrow()
      .quiet()
...
t.it("respects file patterns from command line", async () => {
  ...
  const ourResult = await $`${OURS_BIN} output-file-paths test.ts`
    .cwd(oursDir)
    .nothrow()
    .quiet()

  // Run rust dprint with specific pattern
  const theirResult = await $`${THEIR_BIN} output-file-paths test.ts`
    .cwd(theirsDir)
    .nothrow()
    .quiet()
