...
t.beforeEach(() => {
  ...
  testDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "dprint-test-comparison-cache-"),
  )
...
t.it("both implementations format all files on first run", async () => {
  ...
  const ourExitCode = await FmtCommand.run({ logLevel: "info", cwd: oursDir })

  // Format with rust dprint
  const theirResult = await $`${THEIR_BIN} fmt`.cwd(theirsDir).nothrow().quiet()
  ...
  const expectedFiles = fs
    .readdirSync(oursDir)
    .filter((f) => f.includes(".actual."))
    .length
  t.expect(expectedFiles).toBeGreaterThan(0)
})

t.it(
  "both implementations skip all files on second run (cache hit)",
  async () => {
    ...
    const ourResult = await $`${OURS_BIN} fmt`
      .cwd(oursDir)
      .nothrow()
      .quiet()
...
t.it(
  "both implementations only format changed files after modification",
  async () => {
    // First run - format everything
    await FmtCommand.run({ logLevel: "silent", cwd: oursDir })
    await $`${THEIR_BIN} fmt --log-level silent`.cwd(theirsDir).nothrow()
...
t.it("both implementations invalidate cache when config changes", async () => {
  // First run - format everything
  await FmtCommand.run({ logLevel: "silent", cwd: oursDir })
  await $`${THEIR_BIN} fmt --log-level silent`.cwd(theirsDir).nothrow().quiet()

  // Verify second run with no changes skips files
  const checkResult = await $`${OURS_BIN} fmt`
    .cwd(oursDir)
    .nothrow()
    .quiet()
  ...
  const longLine =
    `const reallyLongVariableName = { property1: "value1", property2: "value2", property3: "value3" };`
  ...
  const ourResult = await $`${OURS_BIN} fmt --log-level info`
    .cwd(oursDir)
    .nothrow()
    .quiet()
  const theirResult = await $`${THEIR_BIN} fmt`.cwd(theirsDir).nothrow().quiet()
...
t.it("both implementations handle --incremental=false CLI flag", async () => {
  ...
  const ourResult = await $`${OURS_BIN} fmt --incremental=false`
    .cwd(oursDir)
    .nothrow()
    .quiet()
  const theirResult = await $`${THEIR_BIN} fmt --incremental=false`
    .cwd(theirsDir)
    .nothrow()
    .quiet()
...
t.it(
  "cache provides significant performance improvement on second run",
  async () => {
    ...
    Ours: ${ourTime1}ms -> ${ourTime2}ms (${
      ((ourTime2 / ourTime1) * 100).toFixed(1)
    }% of original)
    Theirs: ${theirTime1}ms -> ${theirTime2}ms (${
      ((theirTime2 / theirTime1) * 100).toFixed(1)
    }% of original)`)
  },
)
