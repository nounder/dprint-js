...
t.it("returns empty array when no plugins specified and no package.json", async () => {
  ...
  const result = await Formatter.loadPlugins(config, uniqueTempDir, configPath);
...
t.it("returns markdown formatter for .markdown files", async () => {
  ...
  const { plugins: loadedPlugins } = await Formatter.loadPlugins(config);
  const formatter = Formatter.getFormatterForFile("test.markdown", loadedPlugins);
  t.expect(formatter).toBeDefined();
});
...
t.it("formats Async.ts to match expected output", async () => {
  ...
  // Format the file
  const result = await Formatter.formatFile(tempPath, loadedPlugins, config, false);
