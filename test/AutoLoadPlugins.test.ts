...
t.it("auto-loads plugins from package.json when config has no plugins property", async () => {
...
t.it("auto-loads plugins from package.json when config has empty plugins array", async () => {
  ...
  const exitCode = await CheckCommand.run({ cwd: testDir, allowGitignored: true });
