...
t.describe("exclude patterns", () => {
  ...
  t.it("glob patterns with ** work correctly", async () => {
    ...
    fs.writeFileSync(path.join(testDir, "src", "app.ts"), "const x = 1;");
    fs.writeFileSync(path.join(testDir, "src", "test", "app.test.ts"), "const x = 2;");
    fs.writeFileSync(path.join(testDir, "lib", "util.ts"), "const x = 3;");
    fs.writeFileSync(path.join(testDir, "lib", "test", "util.test.ts"), "const x = 4;");
    ...
    const utilTest = fs.readFileSync(path.join(testDir, "lib", "test", "util.test.ts"), "utf-8");
...
t.describe("combined include and exclude patterns", () => {
  t.it("excludes take precedence over includes", async () => {
    ...
    fs.writeFileSync(path.join(testDir, "src", "app.ts"), "const x = 1;");
    fs.writeFileSync(path.join(testDir, "src", "vendor", "lib.ts"), "const x = 2;");
  ...
  t.it("complex pattern combinations work correctly", async () => {
    ...
    fs.writeFileSync(path.join(testDir, "src", "components", "Button.tsx"), "const x = 1;");
    fs.writeFileSync(path.join(testDir, "src", "utils", "helper.ts"), "const x = 2;");
    ...
    const legacy = fs.readFileSync(path.join(testDir, "lib", "legacy.js"), "utf-8");
    const bundle = fs.readFileSync(path.join(testDir, "dist", "bundle.js"), "utf-8");
