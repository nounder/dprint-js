...
t.it("security: vulnerable glob patterns are silently filtered", async () => {
  ...
  try {
    ...
    const files1 = await findFiles(config1, [], path.join(testDir, "project"));
    ...
    const files2 = await findFiles(config2, [], path.join(testDir, "project"));
    ...
    for (const pattern of vulnerablePatterns) {
      ...
      const files = await findFiles(config, [], path.join(testDir, "project"));
